    ...
    if data_type == 'analysis':
        ...
        if trend_data_list:
            ...
            if not combined_trend.empty:
                ...
                with tab_growth:
                    ...
                    if start_year >= end_year:
                        ...
                        for country in filtered_df.columns:
                            ...
                            if start_value > 0: # 시작값이 0보다 클 때만 계산
                                cagr = ((end_value / start_value) ** (1 / num_years) - 1) * 100
                                cagr_results[country] = cagr
                            else:
                                cagr_results[country] = np.nan # 계산 불가 시 NaN 처리
                        ...
                        if cagr_series.empty:
                            ...
                            # 4. 단일 가로 막대그래프 시각화
                            fig = px.bar(
                                x=cagr_series.values,
                                y=cagr_series.index,
                                orientation='h',
                                title=f"주요국 연평균 성장률(CAGR) 비교 ({start_year}–{end_year})",
                                labels={'x': '연평균 성장률 (%)', 'y': '국가'},
                                text_auto='.2f' # 막대 위에 소수점 2자리까지 값 표시
                            )
                            # 막대 색상을 연한 톤으로 변경하고, 값 뒤에 % 추가
                            fig.update_traces(
                                marker_color='#418cdc',
                                texttemplate='%{x:.2f}%',
                ...
                with tab_share_heat:
                    ...
                    fig = px.imshow(share_df.T, text_auto='.1f', aspect="auto", labels=dict(x="발행 연도", y="국가", color="점유율 (%)"), title="주요국 논문 점유율")
                    st.plotly_chart(fig, use_container_width=True)
        st.markdown("---")
        ...
        if collab_df.empty:
            ...
            with tab_trend:
                ...
                min_year = int(df['publication_year'].min()) if not df['publication_year'].empty else 2000
                max_year = int(df['publication_year'].max()) if not df['publication_year'].empty else pd.Timestamp.now().year
                ...
                with col2:
                    end_year = st.number_input("종료 연도", min_value=min_year, max_value=max_year, value=max_year)
                ...
                if start_year > end_year:
                    ...
                    for i, partner_code in enumerate(top_partner_countries):
                        with cols[i % num_columns]:
                            ...
                            trend_data = partner_df.groupby('publication_year').size().reset_index(name='count')
                            trend_data = trend_data[(trend_data['publication_year'] >= start_year) & (trend_data['publication_year'] <= end_year)]

                            fig = px.bar(trend_data, x='publication_year', y='count', title=f'{get_country_name(partner_code)}', )
                            fig.update_xaxes(range=[start_year - 0.5, end_year + 0.5], dtick=1)
                            ...
                            fig.update_layout(
                                ...
                                height=250,
                                bargap=0.3,
                                title_font=dict(        # ✨ 이 부분을 추가/수정합니다.
                                    size=14,            # 폰트 크기를 살짝 줄여서 균형을 맞춥니다.
                                    color='dimgray'     # 진한 검은색 대신 부드러운 회색으로 변경합니다.
                                )
                            )
                            st.plotly_chart(fig, use_container_width=True)
